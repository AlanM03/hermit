...
app = typer.Typer(no_args_is_help=True, help="A local-first AI assistant. For devs, by devs.")
...
def invoke():
    ...
    if not selected_provider_name: raise typer.Exit()
    
    selected_provider = next(provider for provider in providers if provider["name"] == selected_provider_name)
    ...
    if not models:
        coolPrint(f"[bold red]Error:[/bold red] [#DCDCDC]No models found for[/#DCDCDC] [#A0A0A0]{selected_provider_name}[/#A0A0A0][#DCDCDC]. Is the server running?[/#DCDCDC]")
    ...
    if not selected_model: raise typer.Exit()
...
def ponder(prompt: str):
    ...
    with make_api_request(endpoint="/hermit/ponder", payload=payload, stream=True) as response:
...
def scribe():
    ...
    try:
        ...
        diff_process = subprocess.run(git_diff_command, capture_output=True, text=True, check=True, encoding='utf-8')
...
@app.command(name="diagnose", context_settings={"allow_extra_args": True, "ignore_unknown_options": True})
def run_and_diagnose(ctx: typer.Context):
    ...
    try:
        ...
        coolPrint(f"[bold red]Error: Command not found -> '{command_to_run[0]}'.[/bold red]")
        raise typer.Exit(code=1)
    ...
    if return_code != 0:
        ...
        if filepath and os.path.exists(filepath):
            coolPrint(f"[#DCDCDC]Found error in file: {filepath}. Reading for context...[/#DCDCDC]")
        ...
        with make_api_request(endpoint="/hermit/diagnose", payload=payload, stream=True) as response:
            for chunk in response.iter_content(chunk_size=None, decode_unicode=True):
                coolPrint(Text(chunk, style="italic #FFFFFF"), end="", flush=True)
        print("\n")
    else:
