...
from .models import PromptRequest, SaveConfigRequest, ProviderModelRequest, ScribeRequest, ErrorRequest
...
async def get_models_for_provider(request: ProviderModelRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to connect to provider at {request.baseUrl}. Is the server running?")
    
    except Exception as err:
        logging.error(f"Error parsing models from {request.baseUrl}: {err}")
        raise HTTPException(status_code=500, detail="Received an unexpected or invalid response from the provider.")
...
async def ponder(request: PromptRequest):
    ...
    return StreamingResponse(universal_ai_stream(payload, client, config.active_model), media_type="text/plain")
...
async def scribe(request: ScribeRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error communicating with AI provider: {e}")
